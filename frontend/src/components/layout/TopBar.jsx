...
export default function TopBar() {
  ...
  const initialDateStr = new Intl.DateTimeFormat("en-IN", { day: "2-digit", month: "2-digit", year: "numeric" }).format(new Date()).replace(/\//g, " / ");
    ...
    <header style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "20px" }}>
        ...
        <div ref={dateRef} style={{ position: "relative" }}>
          <button
            onClick={() => { setShowDatePicker(v => !v); setShowNotifications(false); setShowApps(false); setShowUserMenu(false); }}
            style={{
              display: "inline-flex", alignItems: "center", gap: "8px",
              background: showDatePicker ? "rgba(37,99,235,0.06)" : "#FFFFFF",
              border: `1px solid ${showDatePicker ? "var(--primary)" : "var(--border-subtle)"}`,
              borderRadius: "var(--radius-md)", padding: "8px 16px",
              fontFamily: "var(--font-mono)", fontSize: "13px", fontWeight: 600,
              color: "var(--text-secondary)", boxShadow: "0 1px 3px rgba(0,0,0,0.04)",
              cursor: "pointer", transition: "all 0.15s ease",
            }}
            title="Click to select system date"
          >
          ...
          {showDatePicker && (
            <div style={{
              ...
              <p style={{ margin: "0 0 10px", fontSize: 12, fontWeight: 700, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.06em" }}>
                Select System Date
              </p>
              <input
                type="date"
                ...
                onChange={(e) => {
                  if (e.target.value) {
                    ...
                    const formatted = new Intl.DateTimeFormat("en-IN", { day: "2-digit", month: "2-digit", year: "numeric" }).format(d).replace(/\//g, " / ");
              ...
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <button
                  onClick={() => {
                    ...
                    const todayFmt = new Intl.DateTimeFormat("en-IN", { day: "2-digit", month: "2-digit", year: "numeric" }).format(new Date()).replace(/\//g, " / ");
                    setSelectedDateStr(todayFmt);
