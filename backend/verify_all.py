...
Tests all REST endpoints, all 5 scenarios, Python tools, decision engine, and multi-step agent runtime.
Supports direct execution via FastAPI TestClient (self-contained, in-memory MongoDB mock).
"""
import sys
sys.path.insert(0, '.')
...
from app.tools.supplier_tools import get_supplier, send_supplier_message, get_tracking_status, request_clarification
...
# Initialize in-memory Mongo mock and seed data
mock_db = mongomock.MongoClient()["supplychaindb_verify"]
seed_run(mock_db)
...
chk("GET /inventory/ (>=8 items)", r.status_code == 200 and len(r.json()) >= 8, f"{len(r.json())} items")
...
chk("GET /suppliers/ (>=8)", r.status_code == 200 and len(r.json()) >= 8, f"{len(r.json())} suppliers")
...
chk("GET /production/PROD-882 (Widget-X)", r.status_code == 200 and "Widget-X" in r.json().get("product", ""), r.json().get("product"))
...
print()
print("=" * 60)
print("3. MULTI-STEP AGENT CONTROLLER & TASKS")
print("=" * 60)

if injected_ids:
    iid = injected_ids[0]
    # Run full agent loop
    r_run = client.post("/agent/run-incident", json={"incident_id": iid}, headers={"X-API-Key": "changeme-secret-key"})
    chk(f"POST /agent/run-incident ({iid})", r_run.status_code == 200 and "tasks" in r_run.json(), f"decision={r_run.json().get('decision')}")

    # Fetch dynamic task decomposition
    r_tasks = client.get(f"/agent/tasks/{iid}")
    chk(f"GET /agent/tasks/{iid}", r_tasks.status_code == 200 and len(r_tasks.json().get("tasks", [])) >= 5, f"{len(r_tasks.json().get('tasks', []))} tasks")

    # Fetch rich audit trail
    r_audit = client.get(f"/agent/audit/{iid}")
    chk(f"GET /agent/audit/{iid}", r_audit.status_code == 200 and len(r_audit.json().get("audit_trail", [])) >= 3, f"{len(r_audit.json().get('audit_trail', []))} audit entries")
...
chk("build_recovery_plan (single + split options)", len(plan.options) >= 2, f"recommended={plan.recommended_option_id}")
...
t = get_inventory("COMP-104", mock_db)
chk("Tool: get_inventory COMP-104", t.success, t.summary[:70])

t = get_supplier("SUP-21", mock_db)
chk("Tool: get_supplier SUP-21", t.success, t.summary[:70])

t = send_supplier_message("SUP-21", "PO-7712", "Please confirm shipment status.", mock_db)
chk("Tool: send_supplier_message", t.success and "reply" in t.data, t.summary[:70])

t = request_clarification("SUP-21", "PO-7712", "Please clarify discrepancy.", "Dispatched", mock_db)
chk("Tool: request_clarification", t.success and "reply" in t.data, t.summary[:70])

t = request_rfq("COMP-104", 600, ["SUP-18", "SUP-42", "SUP-07"], mock_db)
