...
r = httpx.get(f"{BASE}/inventory/COMP-104")
h = r.json()
...
r_adj = httpx.post(f"{BASE}/inventory/COMP-104/adjust", json={"delta": 50, "reason": "test"})
...
for sc in ["SUPPLIER_DELAY", "STALE_INVENTORY", "SUPPLIER_LIE", "QUALITY_FAILURE", "BUDGET_OVERRUN"]:
    r = httpx.post(f"{BASE}/simulator/inject", json={"scenario": sc})
    d = r.json()
...
r = httpx.post(f"{BASE}/simulator/inject", json={"scenario": "INVALID_SCENARIO"})
...
risk = assess_production_risk("PROD-882", 4.33, datetime.utcnow() + timedelta(days=10), "HIGH")
chk("assess_production_risk (high prio, tight supply)", risk.risk_level in ["HIGH", "CRITICAL"], f"risk={risk.risk_level}")

risk_safe = assess_production_risk("PROD-801", 30.0, datetime.utcnow() + timedelta(days=5), "LOW")
...
total = len(results)
failed_tests = [(n, d) for n, ok, *d in [(n, ok, "") for n, ok in results] if not ok]
