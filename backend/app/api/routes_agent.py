...
def approve_plan(
    ...
    db["incidents"].update_one(
        ...
        {"$set": {"status": AgentState.EXECUTING.value}},
    )
    db["audit_logs"].insert_one(
        {"incident_id": decision.incident_id, "action": "Recovery plan approved by coordinator.", "decision": "APPROVED"}
    )
...
def reject_plan(
    ...
    db["audit_logs"].insert_one(
        {"incident_id": decision.incident_id, "action": "Recovery plan rejected; replanning required.", "decision": "REJECTED"}
    )
